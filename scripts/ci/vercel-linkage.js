...
function classifyVercelFailure(text, exitCode, context = '') {
  ...
  if (/\b404\b|not found/.test(normalized) && (context === 'project' || context === 'pull')) return 'PROJECT_NOT_FOUND';
...
function runVercelDiagnostic(command, args, env, { cwd = process.cwd(), runner = spawnSync, requireToken = true, context = '' } = {}) {
  ...
  return {
    status,
    category: classifyVercelFailure(output, status, context),
...
function selectDiagnosticMessage(sanitizedOutput) {
  const lines = String(sanitizedOutput || '').split('\n').filter(Boolean);
  return lines.find((line) => !/^Vercel CLI \d/.test(line) && /error|invalid|not found|unauthorized|forbidden|scope|project|team|\b[1-5]\d\d\b/i.test(line)) || lines.at(-1) || '';
}
...
function runDiagnostic(env = process.env) {
  ...
  const run = (label, args, options = {}) => {
    ...
    const message = selectDiagnosticMessage(result.sanitizedOutput);
    if (message) console.log(`${label}_MESSAGE=${message}`);
    return result;
  };

  const version = run('VERCEL_VERSION', ['--version'], { requireToken: false });
  const whoami = run('WHOAMI', ['whoami']);
  const teams = run('TEAM_ACCESS', ['teams', 'list', '--scope', EXPECTED_ORG_ID], { context: 'team' });
  const project = run('PROJECT_ACCESS', ['project', 'inspect', EXPECTED_PROJECT_ID, '--scope', EXPECTED_ORG_ID], { context: 'project' });
  ...
  let linkedPull;
  try {
    pull = run('PULL_EXACT', ['pull', '--yes', '--environment=production', '--project', EXPECTED_PROJECT_ID, '--scope', EXPECTED_ORG_ID], { cwd: temporaryDirectory, context: 'pull' });
    writeTemporaryProjectLinkage(temporaryDirectory);
    linkedPull = run('PULL_LINKED', ['pull', '--yes', '--environment=production', '--scope', EXPECTED_ORG_ID], { cwd: temporaryDirectory, context: 'pull' });
  } finally {
