...
test('alerting is disabled by default and does not claim delivery', () => {
  const configured = createConfiguredAlerting(env);
  assert.equal(configured.delivery instanceof DisabledAlertDelivery, true);
  const result = configured.evaluate({ event: 'readiness_failure', deploymentEnvironment: 'test' });
  assert.deepEqual(result, { delivered: false, status: 'disabled', aggregateCount: 1 });
});
...
test('test delivery receives only the approved sanitized payload fields', () => {
  ...
  assert.equal(policy.evaluate(fixture).delivered, true);
...
test('unsafe request IDs and complete or query-bearing routes are omitted', () => {
...
test('cooldown suppresses duplicates and expiry permits a later alert', () => {
  let now = Date.parse('2026-07-22T00:00:00.000Z');
  const { delivery, policy } = memoryPolicy({ clock: () => now, cooldownSeconds: 60 });
  assert.equal(policy.evaluate({ event: 'readiness_failure', deploymentEnvironment: 'test' }).status, 'recorded_for_test');
  const duplicate = policy.evaluate({ event: 'readiness_failure', deploymentEnvironment: 'test' });
...
test('ordinary 401 and individual 429 events remain dashboard-only by default', () => {
  const { delivery, policy } = memoryPolicy();
  assert.equal(policy.evaluate({ event: 'http_request', status: 401, route: '/api/v1/employees' }).status, 'dashboard_only');
  assert.equal(policy.evaluate({ event: 'rate_limit_denied', status: 429, route: '/api/v1/auth/login' }).status, 'dashboard_only');
  assert.equal(delivery.getRecords().length, 0);
});

test('approved threshold hooks aggregate safely before alerting', () => {
  const { delivery, policy } = memoryPolicy({ thresholds: { loginFailureSpike: 3, refreshFailureSpike: 2, http429Spike: 2 } });
  assert.equal(policy.evaluate({ event: 'authentication_failure' }).status, 'threshold_pending');
  assert.equal(policy.evaluate({ event: 'authentication_failure' }).status, 'threshold_pending');
  assert.equal(policy.evaluate({ event: 'authentication_failure' }).status, 'recorded_for_test');
  assert.equal(policy.evaluate({ event: 'refresh_failure' }).status, 'threshold_pending');
  assert.equal(policy.evaluate({ event: 'refresh_failure' }).status, 'recorded_for_test');
  assert.equal(policy.evaluate({ event: 'rate_limit_denied' }).status, 'threshold_pending');
  assert.equal(policy.evaluate({ event: 'rate_limit_denied' }).status, 'recorded_for_test');
  assert.equal(delivery.getRecords().length, 3);
});

test('readiness and limiter-store failures produce expected safe alerts', () => {
  const { delivery, policy } = memoryPolicy();
  assert.equal(policy.evaluate({ event: 'readiness_failure', requestId: 'request-one' }).delivered, true);
  assert.equal(policy.evaluate({ event: 'rate_limit_store_unavailable', requestId: 'request-two', route: '/api/v1/auth/login' }).delivered, true);
  assert.deepEqual(delivery.getRecords().map((record) => record.eventCategory), ['readiness_failure', 'rate_limit_store_unavailable']);
});

test('configuration, startup-dependency, and unexpected server failures are policy-approved', () => {
  const { delivery, policy } = memoryPolicy();
  for (const event of ['application_config_invalid', 'startup_dependency_failure', 'unexpected_http_5xx']) {
    assert.equal(policy.evaluate({ event, deploymentEnvironment: 'test' }).delivered, true);
  }
...
test('database-latency and function-timeout hooks remain disabled until configured', () => {
  const disabled = memoryPolicy();
  assert.equal(disabled.policy.evaluate({ event: 'database_latency', durationMs: 2000 }).status, 'dashboard_only');
  assert.equal(disabled.policy.evaluate({ event: 'function_timeout' }).status, 'dashboard_only');
  ...
  assert.equal(enabled.policy.evaluate({ event: 'database_latency', durationMs: 499 }).status, 'dashboard_only');
  assert.equal(enabled.policy.evaluate({ event: 'database_latency', durationMs: 500 }).status, 'recorded_for_test');
  assert.equal(enabled.policy.evaluate({ event: 'function_timeout' }).status, 'threshold_pending');
  assert.equal(enabled.policy.evaluate({ event: 'function_timeout' }).status, 'recorded_for_test');
  assert.deepEqual(enabled.delivery.getRecords().map((record) => record.eventCategory), ['database_latency', 'function_timeout']);
});

test('cleanup alert requires repeated failure and reset clears aggregate state', () => {
  const { delivery, policy } = memoryPolicy();
  assert.equal(policy.evaluate({ event: 'rate_limit_cleanup_failure' }).status, 'threshold_pending');
  assert.equal(policy.evaluate({ event: 'rate_limit_cleanup_failure' }).status, 'recorded_for_test');
  policy.reset();
  assert.equal(policy.evaluate({ event: 'rate_limit_cleanup_failure' }).status, 'threshold_pending');
  assert.equal(delivery.getRecords().length, 1);
});

test('central logger forwards safe records to the policy without forwarding raw inputs', () => {
  const { delivery, policy } = memoryPolicy();
  const lines = [];
