...
test('provider, PAD, match, injection, replay, stale and expiry outcomes map fail-closed', () => {
  const cases = [
    ...
    ['ACTIVE_CHALLENGE_FAILED', 'ACTIVE_CHALLENGE_RETRY', 'RETRY_ACTIVE_CHALLENGE', true],
    ['FACE_MATCH_FAILED', 'FACE_NOT_MATCHED', 'RETRY_FACE_VERIFICATION', true],
    ['FACE_REFERENCE_INVALID', 'REFERENCE_PHOTO_REPLACEMENT_REQUIRED', 'CONTACT_ADMIN', false],
    ['FACE_REFERENCE_NOT_NEUTRAL', 'REFERENCE_PHOTO_REPLACEMENT_REQUIRED', 'CONTACT_ADMIN', false],
    ...
    ['FACE_VERIFICATION_SESSION_ALREADY_ACTIVE', 'VERIFICATION_RESTART_REQUIRED', 'RESTART_VERIFICATION', true],
...
test('device, reference, schedule, site, QR and GPS authority outcomes map to stable remediation states', () => {
  const cases = [
    ...
    ['ATTENDANCE_QR_STEP_UP_REQUIRED', 'QR_STEP_UP_REQUIRED', 'SCAN_CURRENT_SITE_QR'],
