...
test('V3 auth bootstrap removes 25 known F/G Dashboard sources and login suppresses three more without weakening Dashboard coverage', () => {
  ...
  assert.match(auth, /waitForRequest[\s\S]*\/api\/v1\/auth\/login/);
  assert.match(auth, /scrubLoginCredentialDom\(page\)/);
  assert.match(auth, /performAndWaitForHeavyRequest\(page, '\/api\/v1\/dashboard'/);
  assert.match(authenticated, /roleAccessToken\(role\)/);
  assert.match(authenticated, /if \(item\.id === 'dashboard'\) await loginAs\(page, role\);[\s\S]*else await bootstrapAs\(page, role\)/);
...
test('heavy-read safety metrics use only the existing safe stage artifact contract', () => {
  const uatTest = read('e2e/helpers/uat-test.js');
  assert.match(uatTest, /UAT_STAGE_DIAGNOSTIC_FILE/);
  assert.match(uatTest, /outstandingHeavyReadsAtTestEnd/);
  assert.match(uatTest, /exceptionalDrainCount/);
  assert.match(uatTest, /exceptionalDrainWaitMs/);
  const record = JSON.stringify({
    ...
    durationBucket: '<1s',
    safeErrorCode: null,
    outstandingHeavyReadsAtTestEnd: 0,
    exceptionalDrainCount: 0,
    exceptionalDrainWaitMs: 0
  });
  assert.deepEqual(scanArtifact('test-results/uat-v3-stage-diagnostics.jsonl', record), {
    path: 'test-results/uat-v3-stage-diagnostics.jsonl',
    categories: [],
    safe: true
  });
});
