...
test('self-hosted orchestration issues a receipt without retaining routine Attendance face evidence', async () => {
  const c = context();
...
test('successful self-hosted evaluation never invokes configured routine evidence storage', async () => {
  let storeCalls = 0;
  let removeCalls = 0;
  const c = context({
    evidenceStorage: {
...
test('self-hosted receipt authority failure does not invoke routine evidence storage or cleanup', async () => {
  let storeCalls = 0;
  let removeCalls = 0;
  const c = context({
    recordFails: true,
...
test('self-hosted biometric pass without an issued receipt fails closed without retaining routine evidence', async () => {
