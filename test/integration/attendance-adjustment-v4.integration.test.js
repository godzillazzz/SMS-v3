...
if (!configured) {
  ...
  test('returned request cannot be resubmitted until the maker creates a new revision', async () => {
    const draft = await service.createDraft({
      actor: manager,
      assignmentId: ids.assignment,
      requestType: 'ADJUST_WORK_TIME',
      proposal: { checkOutAt: '2099-04-10T11:50:00.000Z' },
      reason: 'Returned request must require a maker revision before resubmission'
    });
    const pending = await service.submit({ actor: manager, id: draft.id });
    assert.equal(pending.status, 'PENDING_APPROVAL');
    assert.equal(pending.currentRevision, 1);

    const returned = await service.returnForCorrection({
      actor: admin,
      id: draft.id,
      comment: 'กรุณาตรวจสอบเวลาออกและแก้ไขก่อนส่งใหม่'
    });
    assert.equal(returned.status, 'RETURNED_FOR_CORRECTION');
    assert.equal(returned.currentRevision, 1);

    await assert.rejects(
      () => service.submit({ actor: manager, id: draft.id }),
      (error) => error.statusCode === 409
        && error.details?.code === 'ATTENDANCE_ADJUSTMENT_REVISION_REQUIRED_AFTER_RETURN'
    );

    const unchanged = await service.get({ actor: manager, id: draft.id });
    assert.equal(unchanged.status, 'RETURNED_FOR_CORRECTION');
    assert.equal(unchanged.currentRevision, 1);

    const revised = await service.revise({
      actor: manager,
      id: draft.id,
      requestType: 'ADJUST_WORK_TIME',
      proposal: { checkOutAt: '2099-04-10T11:45:00.000Z' },
      reason: 'Adjusted after ADMIN review comment'
    });
    assert.equal(revised.status, 'RETURNED_FOR_CORRECTION');
    assert.equal(revised.currentRevision, 2);

    const resubmitted = await service.submit({ actor: manager, id: draft.id });
    assert.equal(resubmitted.status, 'PENDING_APPROVAL');
    assert.equal(resubmitted.currentRevision, 2);

    const detail = await service.get({ actor: manager, id: draft.id });
    assert.deepEqual(
      detail.events.map((event) => event.eventType),
      ['CREATED', 'SUBMITTED', 'RETURNED', 'REVISED', 'SUBMITTED']
    );
  });
