...
test('parses the deployment ID and URL from Vercel JSON output', () => {
  assert.deepEqual(deploymentRecord(JSON.stringify({ id: 'dpl_new123', url: 'sms-v3-staging-new.vercel.app', projectId: 'prj_expected', createdAt: 123, readyState: 'READY' })), {
    id: 'dpl_new123', url: 'https://sms-v3-staging-new.vercel.app', projectId: 'prj_expected', createdAt: 123, readyState: 'READY', source: 'prebuilt'
  });
});

test('rejects a reused rollback deployment and mismatched project', () => {
  const record = deploymentRecord(JSON.stringify({ id: 'dpl_old', url: 'sms-v3-staging-old.vercel.app', projectId: 'prj_expected' }));
