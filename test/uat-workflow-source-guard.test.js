...
test('UAT workflow trusts exact remote branch HEAD/SHA instead of a static application branch allowlist', () => {
  ...
  assert.equal(count('git check-ref-format --branch "$SOURCE_BRANCH"'), 2);
...
test('UAT workflow retains exact source, harness, application checkout, and deployment identity guards in both jobs', () => {
  assert.equal(count('REMOTE_SOURCE_SHA="$(git rev-parse "refs/remotes/origin/$SOURCE_BRANCH")"'), 2);
  assert.equal(count('git rev-parse origin/test/automated-uat-v3-authenticated'), 2);
  assert.equal(count('test "$(git rev-parse HEAD)" = "$HARNESS_SHA"'), 2);
