...
let parsed;
if (isTest) {
  parsed = schema.parse({ ...process.env, DATABASE_URL: process.env.DATABASE_URL || 'postgresql://test:test@localhost:5432/smsv3_test', JWT_SECRET: process.env.JWT_SECRET || 'test-secret-with-at-least-thirty-two-chars' });
} else {
