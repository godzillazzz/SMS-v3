...
export function PersonnelDirectoryPage({ employees, totalCount, loading, error, canManage, role, searchValue, onSearchValueChange, onAdd, onEdit, onLifecycle, onRefresh }: Props) {
  ...
  return <section className="personnel-directory-page" aria-label="Personnel Directory">
    <PersonnelDirectoryHeader canManage={canManage} totalCount={totalCount} onAdd={onAdd} onRefresh={onRefresh} />
    ...
    <div className="personnel-summary-grid"><PersonnelMetricCard icon="◉" label="บุคลากรทั้งหมด" value={totalCount} context="รายการที่เข้าถึงได้" tone="indigo" /><PersonnelMetricCard icon="✓" label="บุคลากรที่ใช้งาน" value={employees.filter((employee) => employee.isActive).length} context="กำลังปฏิบัติงาน" tone="green" /><PersonnelMetricCard icon="⌘" label="โปรไฟล์ไม่สมบูรณ์" value={incomplete} context={incomplete ? 'ต้องตรวจสอบข้อมูล' : 'ข้อมูลครบถ้วน'} tone="amber" /><PersonnelMetricCard icon="◌" label="รอตรวจสอบ" context="ยังไม่มีข้อมูล" tone="blue" /></div>
    {permissionDenied ? <div className="personnel-empty-state"><span>⛨</span><h2>ไม่มีสิทธิ์เข้าถึงข้อมูล</h2><p>บัญชีนี้ไม่ได้รับอนุญาตให้ดู Personnel Directory</p></div>
      : error ? <div className="personnel-empty-state"><span>!</span><h2>ไม่สามารถโหลดข้อมูลบุคลากร</h2><p>ระบบไม่พร้อมใช้งานชั่วคราว กรุณาลองใหม่อีกครั้ง</p><button type="button" className="btn-neutral small-action" onClick={onRefresh}>ลองใหม่</button></div>
        : loading ? <div className="personnel-loading" role="status">กำลังอ่านข้อมูลพนักงาน…</div>
          : <><div className="personnel-result-line">แสดง {visible.length} จาก {filtered.length} รายการ{search || department || status ? ' · กรองแล้ว' : ''}</div><PersonnelTable rows={visible} canManage={canManage} role={role} selectedId={selected?.id} onSelect={(employee) => { lastSelectedId.current = employee.id; setSelected(employee); }} onEdit={onEdit} onLifecycle={onLifecycle} /><PersonnelPagination page={page} totalPages={totalPages} onChange={setPage} /></>}
    {!loading && !error && employees.length === 0 && <div className="personnel-empty-state"><span>♙</span><h2>ยังไม่พบข้อมูลบุคลากร</h2><p>ระบบยังไม่พบข้อมูลที่สามารถแสดงภายใต้หน่วยงานและสิทธิ์ของบัญชีนี้</p><button type="button" className="btn-neutral small-action" onClick={onRefresh}>รีเฟรช</button></div>}
    {!loading && !error && employees.length > 0 && filtered.length === 0 && <div className="personnel-empty-state personnel-empty-state--search"><span>⌕</span><h2>ไม่พบผลการค้นหา</h2><p>ลองล้างตัวกรองหรือใช้คำค้นหาอื่น</p><button type="button" className="btn-neutral small-action" onClick={clear}>ล้างตัวกรอง</button></div>}
