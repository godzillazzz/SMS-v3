...
export function AuditCompliancePage({ rows, total, page = 1, totalPages = 1, pageSize = 25, loading, error, permissionDenied, filters, onFiltersChange, onRefresh, onPageChange, onPageSize, onExport, onPrint }: Props) {
  ...
  return <section className="audit-compliance-page data-surface-page" aria-label="บันทึกการใช้งานระบบ"><AuditPageHeader total={total} onRefresh={onRefresh} onExport={() => onExport(rows)} onPrint={onPrint} authorized={!permissionDenied} loading={loading} hasError={Boolean(error)} hasRows={rows.length > 0} />{!permissionDenied && !error && <AuditSummaryGrid total={total} pageCount={rows.length} categories={categories} actors={actors} loading={loading} />}{permissionDenied ? <div className="audit-state audit-state--permission data-state data-state--permission"><span aria-hidden="true">⛨</span><h2>ไม่มีสิทธิ์เข้าถึงบันทึกการใช้งานระบบ</h2><p>เฉพาะผู้ดูแลระบบเท่านั้นที่สามารถดู Audit Log ได้</p></div> : <>{!error && <AuditToolbar pageSize={pageSize} onPageSize={onPageSize} filters={filters} onFiltersChange={onFiltersChange} onClear={() => onFiltersChange({ dateFrom: '', dateTo: '', actor: '', entityType: '', action: '', search: '', category: 'default' })} />}<AuditTable rows={rows} selected={selected} onSelect={setSelected} loading={loading} hasActiveFilters={hasActiveFilters} error={Boolean(error)} onRetry={onRefresh} />{!error && <DataTablePagination page={page} totalPages={totalPages} onChange={onPageChange} ariaLabel="แบ่งหน้า Audit Log" className="audit-pagination" loading={loading} />}</>}<AuditEventPreview event={selected} onClose={() => setSelected(undefined)} /></section>;
}
