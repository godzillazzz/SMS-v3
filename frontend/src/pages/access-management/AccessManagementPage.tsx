...
function AccountTable({ rows, role, originalUserId, loading, onDetails, onEdit, onApprove, onReset, onViewAs, onToggle }: {
  ...
  if (!rows.length) return <div className="access-state data-state data-state--empty"><span aria-hidden="true">♙</span><h2>ยังไม่มีบัญชีที่แสดงได้</h2><p>เมื่อมีบัญชีอยู่ในขอบเขตสิทธิ์ของคุณ ระบบจะแสดงรายการที่นี่</p></div>;
...
export function AccessManagementPage({ rows, loading, error, role, originalUserId, onRefresh, onUpdate, onResetPassword, onViewAs, onOpenAudit }: Props) {
  ...
  return <section className="access-management-page data-surface-page" aria-label="ผู้ใช้และสิทธิ์"><header className="access-page-header"><div><p className="access-eyebrow">{manager ? 'PENDING ACCOUNT APPROVAL' : 'ACCOUNT AND ACCESS MANAGEMENT'}</p><h1>ผู้ใช้และสิทธิ์</h1><p>{manager ? 'รายการที่รออนุมัติ — อนุมัติได้เฉพาะบัญชี Viewer ที่เปิดใช้งาน' : 'บัญชี บทบาท สถานะ และความปลอดภัยของการเข้าถึง'}</p></div><div className="access-page-actions"><span className="access-result-count data-result-count">{loading ? 'กำลังโหลดรายการ…' : `${rows.length} บัญชีในขอบเขตที่แสดง`}</span><button type="button" className="btn-neutral" disabled={loading} onClick={onRefresh}>↻ รีเฟรช</button></div></header>{notice && <div className="access-notice" role="status">✓ {notice}<button type="button" onClick={() => setNotice(undefined)} aria-label="ปิดข้อความ">×</button></div>}{permissionDenied ? <div className="access-state access-state--permission data-state data-state--permission"><span aria-hidden="true">⛨</span><h2>คุณไม่มีสิทธิ์จัดการบัญชี</h2><p>บัญชีนี้ไม่ได้รับอนุญาตให้เข้าถึงผู้ใช้และสิทธิ์</p></div> : <>{state !== 'error' && <AccessSummaryCards rows={rows} manager={manager} />}{state === 'error' ? <div className="access-state data-state data-state--error"><span aria-hidden="true">!</span><h2>ไม่สามารถโหลดข้อมูลบัญชี</h2><p>ระบบไม่พร้อมใช้งานชั่วคราว กรุณาลองใหม่อีกครั้ง</p><button type="button" className="btn-neutral" onClick={onRefresh}>ลองใหม่</button></div> : <AccountTable rows={rows} role={role} originalUserId={originalUserId} loading={loading} onDetails={openDetails} onEdit={(account) => prepare(account, 'edit')} onApprove={approve} onReset={(account) => prepare(account, 'reset')} onViewAs={(account) => prepare(account, 'view-as')} onToggle={updateToggle} />}</>}<AccountDrawer account={selected} role={role} originalUserId={originalUserId} suspendEscape={Boolean(dialog)} onClose={closeDetails} onEdit={(account) => prepare(account, 'edit')} onApprove={approve} onReset={(account) => prepare(account, 'reset')} onViewAs={(account, trigger) => prepare(account, 'view-as', trigger)} onToggle={updateToggle} onOpenAudit={() => { closeDetails(); onOpenAudit(); }} />
