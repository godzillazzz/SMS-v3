...
describe('CFG-UX-01 Security Site governance UX',()=>{
 ...
 it('requires an explicit impact confirmation before destructive Site or QR mutation',()=>{expect(panel).toContain("setPendingDestructiveAction('DEACTIVATE_SITE')");expect(panel).toContain("setPendingDestructiveAction('REVOKE_QR')");expect(panel).toContain('role="dialog"');expect(panel).toContain('aria-modal="true"');expect(panel).toContain('ยืนยันปิดใช้งาน Security Site');expect(panel).toContain('ยืนยัน Revoke QR ปัจจุบัน');expect(panel).toContain('ผลกระทบ');expect(panel).toContain('QR Version');expect(panel).toContain('เหตุผลที่บันทึกใน Audit');expect(panel).not.toContain('window.confirm');expect(css).toContain('.security-site-confirm-backdrop');});
});
