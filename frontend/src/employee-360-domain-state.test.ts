...
describe('EMP-UX Employee 360 authoritative domain states',()=>{
 ...
 it('surfaces account and expiring-license state without inventing device authority',()=>{expect(drawer).toContain('บัญชีผู้ใช้');expect(drawer).toContain('ใกล้หมดอายุภายใน 30 วัน');expect(drawer).toContain('Attendance Device');expect(drawer).toContain('ยังไม่มี employee-target read API');expect(drawer).toContain('ไม่สรุปหรือคาดเดาสถานะอุปกรณ์');});
