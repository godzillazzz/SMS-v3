...
export const api = {
  ...
  uploadLicenseDocument: (token: string, licenseId: string, data: { proposedStartDate: string; proposedExpiryDate: string; note?: string }, document: File) => { const body = new FormData(); body.append('proposedStartDate', data.proposedStartDate); body.append('proposedExpiryDate', data.proposedExpiryDate); if (data.note) body.append('note', data.note); body.append('document', document); return callMultipart(`/licenses/${licenseId}/documents`, token, body); },
