...
export const api = {
  ...
  attendanceSites: (token: string, includeInactive = true) => call(`/attendance-foundation/sites${includeInactive ? '?includeInactive=true' : ''}`, { headers: { Authorization: `Bearer ${token}` } }),
  createAttendanceSite: (token: string, data: unknown) => call('/attendance-foundation/sites', { method: 'POST', body: JSON.stringify(data), headers: { Authorization: `Bearer ${token}` } }),
  updateAttendanceSite: (token: string, id: string, data: unknown) => call(`/attendance-foundation/sites/${id}`, { method: 'PUT', body: JSON.stringify(data), headers: { Authorization: `Bearer ${token}` } }),
  attendanceDuties: (token: string, includeInactive = true) => call(`/attendance-foundation/duties${includeInactive ? '?includeInactive=true' : ''}`, { headers: { Authorization: `Bearer ${token}` } }),
  createAttendanceDuty: (token: string, data: unknown) => call('/attendance-foundation/duties', { method: 'POST', body: JSON.stringify(data), headers: { Authorization: `Bearer ${token}` } }),
  updateAttendanceDuty: (token: string, id: string, data: unknown) => call(`/attendance-foundation/duties/${id}`, { method: 'PUT', body: JSON.stringify(data), headers: { Authorization: `Bearer ${token}` } }),
