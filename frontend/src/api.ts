...
export const api = {
  ...
  createLeaveQuota: (token: string, data: { employeeId: string; sickLeave: number; personalLeave: number; vacationLeave: number }) => call('/leave-quotas', { method: 'POST', body: JSON.stringify(data), headers: { Authorization: `Bearer ${token}` } }),
