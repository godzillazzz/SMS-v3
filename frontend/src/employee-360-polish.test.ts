...
describe('EMP-UX-02 Employee 360 polish',()=>{
  ...
  it('uses fail-safe attendance readiness instead of claiming READY without all authorities',()=>{expect(drawer).toContain('ต้องตรวจสอบเพิ่มเติม');expect(drawer).toContain('Schedule, Site, Shift และ Attendance Device');expect(drawer).toContain('ยังไม่มี employee-target read API');expect(drawer).not.toContain("label: 'READY'");});
