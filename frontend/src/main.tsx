...
import { LEAVE_QUOTA_DEFAULTS, buildLeaveQuotaProvisioningPayload, canProvisionLeaveQuota, hasUnmatchedLegacyQuota, quotaProvisioningEmployeeOptions } from './leave-quota-provisioning';
...
function Dashboard() {
  ...
  const quotaEmployeeOptions = quotaProvisioningEmployeeOptions(employees, quotaRows);
  const showQuotaLegacyWarning = Number(operationResponse.meta?.unmatchedLegacyCount || 0) > 0 || hasUnmatchedLegacyQuota(quotaRows);
  ...
  const openCreateOperation = () => {
    ...
    if (activePage === 'quota' && auth.user?.role === 'ADMIN') runEditor({
      title: 'กำหนดโควตาวันลา', submitLabel: 'บันทึกโควตา',
      ...
      values: { ...LEAVE_QUOTA_DEFAULTS }
    }, (form) => api.createLeaveQuota(auth.token!, buildLeaveQuotaProvisioningPayload(form)));
