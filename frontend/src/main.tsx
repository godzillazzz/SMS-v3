        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="dialog-actions">
            ...
            <button className="btn-primary compact wand-submit-btn" type="submit" disabled={busy}>
              {busy ? 'กำลังคำนวณแพทเทิร์น…' : '🪄 ใส่ลงในฉบับร่าง (ยังไม่บันทึก)'}
            </button>
          </div>
        </form>
      </section>
...
const tablePages: Record<Exclude<Page, 'dashboard' | 'employees' | 'reports' | 'shiftSetup' | 'settings'>, { title: string; eyebrow: string; description: string; columns: Array<{ label: string; value: (row: DataRow) => React.ReactNode }> }> = {
...
function Dashboard() {
  ...
  useEffect(() => {
    if (!auth.token || activePage !== 'leave') return;
  ...
  const content = () => {
    ...
    if (activePage === 'schedule') {
      ...
      const applyPreviewToDrafts = (previewRowsParam: DataRow[]) => {
        ...
        for (const row of previewRowsParam) {
          ...
          newDrafts[key] = {
            ...
            remark: 'จัดด้วยไม้กายสิทธิ์ (ฉบับร่าง)',
            payload: {
              employeeId: empId,
              workDate: workDateStr,
              shiftTypeId: String(selectedType?.id || row.shiftTypeId || ''),
      ...
      return <section className="view-pane schedule-calendar-page">
        ...
        {canManage && (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '10px 0', padding: '10px 16px', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '10px' }}>
            <span style={{ fontSize: '13.5px', color: '#334155' }}>
              💡 <b>โหมดบันทึกตารางกะด้วยตนเอง:</b> ระบบจะไม่สั่ง Auto-Refresh หน้าจอชั่วคราวขณะแก้ไข เมื่อคลิกแก้ไข/เพิ่ม/ลบกะ ให้กดปุ่ม <b>"💾 บันทึกการเปลี่ยนแปลง"</b> ด้านขวาเพื่อส่งบันทึกทีเดียว
            </span>
            {Object.keys(scheduleDrafts).length > 0 ? (
              <button className="btn-primary compact" style={{ backgroundColor: '#16a34a', borderColor: '#15803d', fontWeight: 'bold', fontSize: '14px' }} disabled={batchSaveBusy} onClick={saveAllDrafts}>
                💾 บันทึกการเปลี่ยนแปลง ({Object.keys(scheduleDrafts).length})
              </button>
            ) : (
              <span style={{ fontSize: '12.5px', color: '#64748b', fontStyle: 'italic' }}>ยังไม่มีรายการแก้ไขในร่าง</span>
            )}
          </div>
        )}
        {auth.user?.role === 'ADMIN' && <section className="auto-schedule-panel"><div className="auto-schedule-heading"><div><span className="auto-schedule-icon">🪄</span><div><h2>จัดตารางกะอัตโนมัติ</h2><p>เครื่องมือไม้กายสิทธิ์สำหรับ Admin — สร้างตัวอย่างก่อนบันทึก โดยคงกะที่ล็อกและวันลา (AL)</p></div></div><button className="btn-primary compact" disabled={autoScheduleBusy} onClick={previewAutoSchedule}>{autoScheduleBusy ? 'กำลังคำนวณ…' : '🪄 ดูตัวอย่างจัดกะอัตโนมัติ'}</button></div>{autoSchedulePreview && <div className="auto-schedule-preview"><div className="preview-summary"><span><b>{text(previewSummary.employees)}</b> พนักงาน</span><span><b>{text(previewSummary.totalRows)}</b> กะทั้งหมด</span><span><b>{text(previewSummary.manualLocked)}</b> รายการที่คงไว้</span><span><b>{previewWarnings.length}</b> คำเตือน</span></div>{previewWarnings.length > 0 && <div className="preview-warning"><strong>รายการที่ต้องตรวจสอบ</strong>{previewWarnings.slice(0, 8).map((warning, index) => <p key={`${String(warning)}-${index}`}>• {text(warning)}</p>)}</div>}<div className="table-scroll preview-table-wrap"><table className="data-table preview-table"><thead><tr><th>พนักงาน</th><th>วันที่</th><th>กะ</th><th>เหตุผล</th></tr></thead><tbody>{previewRows.slice(0, 20).map((row, index) => <tr key={`${text(row.employeeId)}-${text(row.date)}-${index}`}><td>{text(row.employeeName)}</td><td>{date(row.date)}</td><td><span className={`status-badge ${row.code === 'OFF' ? 'inactive' : 'active'}`}>{text(row.code)}</span></td><td>{text(row.remark)}</td></tr>)}</tbody></table></div>{previewRows.length > 20 && <small className="preview-more">แสดงตัวอย่าง 20 จาก {previewRows.length} รายการ</small>}<div className="preview-actions"><button className="btn-secondary" disabled={autoScheduleBusy} onClick={() => setAutoSchedulePreview(undefined)}>ยกเลิก Preview</button><button className="btn-primary compact" disabled={autoScheduleBusy} onClick={saveAutoSchedule}>ยืนยันบันทึกตาราง</button></div></div>}</section>}
        {Object.keys(scheduleDrafts).length > 0 && (
          <div className="batch-save-bar alert alert-info" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '12px 0', padding: '12px 18px', background: '#eff6ff', border: '2px solid #3b82f6', borderRadius: '10px', boxShadow: '0 4px 12px rgba(59, 130, 246, 0.15)' }}>
            <div>
              <strong style={{ color: '#1e40af', fontSize: '15px' }}>💾 มีรายการจัดกะในฉบับร่าง ({Object.keys(scheduleDrafts).length} รายการ)</strong>
              <p style={{ margin: '2px 0 0', color: '#1d4ed8', fontSize: '13px' }}>ตารางจะไม่ถูกรีเฟรชหรือส่งไปเซิร์ฟเวอร์จนกว่าคุณจะกดปุ่ม "บันทึกการเปลี่ยนแปลง"</p>
            </div>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button className="btn-secondary compact" disabled={batchSaveBusy} onClick={() => setScheduleDrafts({})}>✖ ยกเลิกทั้งหมด</button>
              <button className="btn-primary compact" style={{ backgroundColor: '#16a34a', borderColor: '#15803d', fontWeight: 'bold', fontSize: '14px', padding: '8px 16px' }} disabled={batchSaveBusy} onClick={saveAllDrafts}>
                {batchSaveBusy ? 'กำลังบันทึก…' : `💾 บันทึกการเปลี่ยนแปลง (${Object.keys(scheduleDrafts).length})`}
              </button>
            </div>
          </div>
        )}
    ...
    if (activePage === 'rules') {
      ...
      return <section className="view-pane"><div className="page-heading"><div><p className="eyebrow">ตารางและกฎการทำงาน</p><h1>Rule Checking</h1><p>ตรวจสอบกฎเดิมกับตารางกะจาก PostgreSQL แบบ read-only</p></div><div className="heading-actions"><label className="month-filter"><span>เดือน</span><input type="month" value={scheduleMonth} onChange={(event) => setScheduleMonth(event.target.value)} /></label><button className="small-action" onClick={() => setOperationRefresh((value) => value + 1)}>ตรวจสอบอีกครั้ง</button></div></div>
        {operationError && <div className="alert alert-error">{operationError}</div>}
