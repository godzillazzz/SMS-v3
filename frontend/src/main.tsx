...
function Login() {
  ...
  return (
    <main className="login-page auth-experience-page award-auth-page">
      ...
      <header className="award-public-nav"><div className="award-public-brand"><Logo /><span><b>SMS</b><small>SECURITY OPERATIONS</small></span></div><nav><a href="#award-capabilities">Capabilities</a><a href="#award-trust">Trust</a><a href="#auth-login-form" className="award-nav-cta">เข้าสู่ระบบ</a></nav></header>
      {mode === 'login' && <section className="award-hero"><div className="award-hero-copy"><p className="award-kicker"><span /> SECURITY INTELLIGENCE · UNIFIED OPERATIONS</p><h1>See the operation.<br /><em>Secure the moment.</em></h1><p className="award-hero-lead">ศูนย์กลางการบริหารงานรักษาความปลอดภัยที่เชื่อมบุคลากร ตารางปฏิบัติงาน การลงเวลา การอนุมัติ และข้อมูลเชิงปฏิบัติการไว้ในประสบการณ์เดียว</p><div className="award-hero-actions"><a className="award-primary" href="#auth-login-form">เข้าสู่ระบบ <span>↗</span></a><a className="award-secondary" href="#award-capabilities">สำรวจแพลตฟอร์ม</a></div><div className="award-trustline" id="award-trust"><span><b>01</b> Role-based access</span><span><b>02</b> Operational visibility</span><span><b>03</b> Audit-ready workflow</span></div></div><div className="award-command-visual" aria-hidden="true"><div className="award-orbit award-orbit-a" /><div className="award-orbit award-orbit-b" /><div className="award-radar"><div className="award-radar-grid" /><div className="award-radar-sweep" /><span className="award-node n1" /><span className="award-node n2" /><span className="award-node n3" /><span className="award-node n4" /><div className="award-core"><Logo /><b>SMS</b><small>LIVE OPERATIONS</small></div></div><div className="award-float-card award-float-card-a"><span className="award-live-dot" /><div><small>OPERATION STATUS</small><b>Protected & connected</b></div></div><div className="award-float-card award-float-card-b"><small>WORKFORCE</small><b>Unified command</b><span>Personnel · Schedule · Attendance</span></div></div></section>}
      {mode === 'login' && <section className="award-capabilities" id="award-capabilities"><article><span>01</span><SmsIcon name="employees" size={22} /><h2>People</h2><p>บริหารข้อมูลบุคลากรและสิทธิ์อย่างเป็นระบบ</p></article><article><span>02</span><SmsIcon name="calendar" size={22} /><h2>Operations</h2><p>วางตารางกะ ลงเวลา และติดตามงานใน flow เดียว</p></article><article><span>03</span><SmsIcon name="approval" size={22} /><h2>Decisions</h2><p>รวม approval workflow และสถานะที่ต้องจัดการ</p></article><article><span>04</span><SmsIcon name="shield" size={22} /><h2>Assurance</h2><p>สิทธิ์ การตรวจสอบ และหลักฐานพร้อมสำหรับการกำกับดูแล</p></article></section>}
    ...
    <div className="employee-magic-wand-modal__viewport" role="presentation" onMouseDown={handleBackdropMouseDown}>
      <section className="employee-magic-wand-modal__dialog magic-wand-dialog" role="dialog" aria-modal="true" aria-labelledby="magic-wand-title" onMouseDown={(event) => event.stopPropagation()}>
        <div className="dialog-heading">
          ...
          <button type="button" aria-label="ปิด" disabled={busy} onClick={onClose}><SmsIcon name="close" size={18} /></button>
        </div>
...
function OperationalTable({ page, response, loading, error, onPageChange, onAction, onCreate, onNavigate, role, token, refreshSignal, onLicenseDocumentChanged, onEditLicense, licenseEmployeeStatus = 'ACTIVE', onLicenseEmployeeStatusChange }: { page: OperationalPage; response: DataResponse; loading: boolean; error?: RequestErrorInput; onPageChange(page: number): void; onAction(row: DataRow, action: string): void; onCreate(): void; onNavigate(page: Page): void; role: string; token?: string; refreshSignal: number; onLicenseDocumentChanged(message: string): void; onEditLicense?: (row: DataRow) => void; licenseEmployeeStatus?: LicenseEmployeeStatus; onLicenseEmployeeStatusChange?(status: LicenseEmployeeStatus): void }) {
  ...
  const licenseTableHeader = <tr>{config.columns.map((column) => <th key={column.label} scope="col">{column.label}</th>)}{showActions && <TableActionHeader label="ดำเนินการ" />}</tr>;
  const licenseEmptyState = <DataTableState
    variant="empty"
    title={noResultsMessage}
    description={page === 'licenses' && tableSearch ? 'ลองเปลี่ยนคำค้นหา หรือล้างตัวกรองแล้วค้นหาอีกครั้ง' : 'ยังไม่มีรายการที่ต้องดำเนินการในขอบเขตนี้'}
    action={page === 'licenses' && tableSearch ? { label: 'ล้างคำค้นหา', onClick: () => setTableSearch('') } : canCreate ? { label: createLabel, onClick: onCreate } : undefined}
  />;
  const licenseDesktop = error
    ? <DataTableState variant="error" title="ไม่สามารถโหลดใบอนุญาต" description="ข้อมูลใบอนุญาตยังไม่พร้อมใช้งาน" />
    : loading
      ? <div className="table-scroll data-table-scroll"><table className="data-table data-surface-table signature-data-table" aria-label="รายการใบอนุญาตพนักงาน"><thead>{licenseTableHeader}</thead><tbody><DataTableSkeletonRows columnCount={config.columns.length + (showActions ? 1 : 0)} /></tbody></table></div>
      : <div className="table-scroll data-table-scroll"><table className="data-table data-surface-table signature-data-table" aria-label="รายการใบอนุญาตพนักงาน"><thead>{licenseTableHeader}</thead><tbody>{visibleRows.length ? visibleRows.map((row, index) => <tr key={text(row.id) + index} className="signature-data-row" data-operational-row={text(row.id)} tabIndex={0} aria-label={`เปิดรายละเอียดใบอนุญาต ${text(nested(row.employee).firstName)} ${text(nested(row.employee).lastName)}`.trim()} onClick={() => selectRow(row)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); selectRow(row); } }}>{config.columns.map((column) => page === 'licenses' ? renderLicenseCell(row, column) : <td key={column.label}>{tableValue(row, column)}</td>)}{showActions && <TableActionCell className="row-actions data-row-actions" onClick={(event) => event.stopPropagation()}>{rowActions(row)}</TableActionCell>}</tr>) : <tr><td colSpan={config.columns.length + (showActions ? 1 : 0)} className="no-rows data-table-empty-cell">{licenseEmptyState}</td></tr>}</tbody></table></div>;
  const licenseMobile = error
    ...
    : loading
      ...
      : <div className="signature-mobile-records">{visibleRows.map((row) => <button type="button" key={`mobile-${text(row.id)}`} className="signature-mobile-record" data-operational-row={text(row.id)} aria-label={`เปิดรายละเอียดใบอนุญาต ${text(nested(row.employee).firstName)} ${text(nested(row.employee).lastName)}`.trim()} onClick={() => selectRow(row)}><span className="signature-mobile-record__eyebrow">{config.eyebrow}</span><strong>{`${text(nested(row.employee).firstName)} ${text(nested(row.employee).lastName)}`.trim() || text(row.licenseNumber)}</strong><div>{config.columns.filter((column) => column.label !== 'ดูไฟล์').slice(0, 5).map((column) => <span key={column.label}><small>{column.label}</small>{tableValue(row, column)}</span>)}</div><em>แตะเพื่อเปิดรายละเอียด</em></button>)}</div>;
  const quotaTableHeader = <tr>{config.columns.map((column) => <th key={column.label} scope="col">{column.label}</th>)}{showActions && <TableActionHeader label="ดำเนินการ" />}</tr>;
  const quotaEmptyState = <DataTableState
    variant="empty"
    title={noResultsMessage}
    description="ยังไม่มีรายการโควตาในขอบเขตที่เลือก หรือข้อมูลเดิมยังไม่พร้อมแสดง"
    action={canCreate ? { label: createLabel, onClick: onCreate } : undefined}
  />;
  const quotaDesktop = error
    ? <DataTableState variant="error" title="ไม่สามารถโหลดโควตาวันลา" description="ข้อมูลสิทธิ์รายปียังไม่พร้อมใช้งาน" />
    : loading
      ? <div className="table-scroll data-table-scroll"><table className="data-table data-surface-table signature-data-table" aria-label="รายการโควตาวันลา"><thead>{quotaTableHeader}</thead><tbody><DataTableSkeletonRows columnCount={config.columns.length + (showActions ? 1 : 0)} /></tbody></table></div>
      : <div className="table-scroll data-table-scroll"><table className="data-table data-surface-table signature-data-table" aria-label="รายการโควตาวันลา"><thead>{quotaTableHeader}</thead><tbody>{visibleRows.length ? visibleRows.map((row, index) => <tr key={text(row.id) + index} className="signature-data-row" data-operational-row={text(row.id)} tabIndex={0} aria-label={`เปิดรายละเอียดโควตา ${text(row.employeeNameSnapshot) || text(row.employeeId) || text(row.id)}`} onClick={() => selectRow(row)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); selectRow(row); } }}>{config.columns.map((column) => <td key={column.label}>{tableValue(row, column)}</td>)}{showActions && <TableActionCell className="row-actions data-row-actions" onClick={(event) => event.stopPropagation()}>{rowActions(row)}</TableActionCell>}</tr>) : <tr><td colSpan={config.columns.length + (showActions ? 1 : 0)} className="no-rows data-table-empty-cell">{quotaEmptyState}</td></tr>}</tbody></table></div>;
  const quotaMobile = error
    ...
    : loading
      ...
      : <div className="signature-mobile-records">{visibleRows.map((row) => <button type="button" key={`mobile-${text(row.id)}`} className="signature-mobile-record" data-operational-row={text(row.id)} aria-label={`เปิดรายละเอียดโควตา ${text(row.employeeNameSnapshot) || text(row.employeeId) || text(row.id)}`} onClick={() => selectRow(row)}><span className="signature-mobile-record__eyebrow">{config.eyebrow}</span><strong>{text(row.employeeNameSnapshot) || (row.employeeId ? text(row.employeeId) : 'ข้อมูลโควตาเดิม — ยังไม่จับคู่พนักงาน')}</strong><div>{config.columns.slice(0, 6).map((column) => <span key={column.label}><small>{column.label}</small>{tableValue(row, column)}</span>)}</div><em>แตะเพื่อเปิดรายละเอียดและการจัดการ</em></button>)}</div>;
  const quotaSurface = <ResponsiveDataTable ariaLabel="รายการโควตาวันลา" loading={loading} error={Boolean(error)} loadingLabel="กำลังอ่านข้อมูลโควตาวันลา…" errorLabel="ไม่สามารถโหลดโควตาวันลา" className="signature-data-surface" desktop={quotaDesktop} mobile={quotaMobile} />;
  const licenseSurface = <ResponsiveDataTable ariaLabel="รายการใบอนุญาตพนักงาน" loading={loading} error={Boolean(error)} loadingLabel="กำลังอ่านข้อมูลใบอนุญาต…" errorLabel="ไม่สามารถโหลดใบอนุญาต" className="signature-data-surface" desktop={licenseDesktop} mobile={licenseMobile} />;
  const approvalTableHeader = <tr>{config.columns.map((column) => <th key={column.label} scope="col">{column.label}</th>)}{showActions && <TableActionHeader label="ดำเนินการ" />}</tr>;
  const approvalEmptyState = <DataTableState variant="empty" title="ยังไม่มีประวัติการอนุมัติตาราง" description="ยังไม่มีรายการ revision การอนุมัติตารางกะในขอบเขตนี้" />;
  const approvalRowLabel = (row: DataRow) => `เปิดรายละเอียดการอนุมัติตาราง ${date(row.month)} Revision ${text(row.revision)}`;
  const approvalDesktop = error
    ? <DataTableState variant="error" title="ไม่สามารถโหลดประวัติการอนุมัติ" description="ข้อมูลประวัติการอนุมัติตารางยังไม่พร้อมใช้งาน" />
    : loading
      ? <div className="table-scroll data-table-scroll"><table className="data-table data-surface-table signature-data-table" aria-label="รายการประวัติการอนุมัติตารางกะ"><thead>{approvalTableHeader}</thead><tbody><DataTableSkeletonRows columnCount={config.columns.length + (showActions ? 1 : 0)} /></tbody></table></div>
      : <div className="table-scroll data-table-scroll"><table className="data-table data-surface-table signature-data-table" aria-label="รายการประวัติการอนุมัติตารางกะ"><thead>{approvalTableHeader}</thead><tbody>{visibleRows.length ? visibleRows.map((row, index) => <tr key={text(row.id) + index} className="signature-data-row" data-operational-row={text(row.id)} tabIndex={0} aria-label={approvalRowLabel(row)} onClick={() => selectRow(row)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); selectRow(row); } }}>{config.columns.map((column) => <td key={column.label}>{tableValue(row, column)}</td>)}{showActions && <TableActionCell className="row-actions data-row-actions" onClick={(event) => event.stopPropagation()}>{rowActions(row)}</TableActionCell>}</tr>) : <tr><td colSpan={config.columns.length + (showActions ? 1 : 0)} className="no-rows data-table-empty-cell">{approvalEmptyState}</td></tr>}</tbody></table></div>;
  const approvalMobile = error
    ...
    : loading
      ...
      : <div className="signature-mobile-records approval-mobile-records">{visibleRows.length ? visibleRows.map((row) => <article key={`mobile-${text(row.id)}`} className="signature-mobile-record approval-mobile-record" data-operational-row={text(row.id)}><button type="button" className="approval-mobile-record__open" aria-label={approvalRowLabel(row)} onClick={() => selectRow(row)}><span className="signature-mobile-record__eyebrow">{config.eyebrow}</span><strong>{`ตาราง ${date(row.month)} · Revision ${text(row.revision)}`}</strong><div>{config.columns.map((column) => <span key={column.label}><small>{column.label}</small>{tableValue(row, column)}</span>)}</div><em>แตะเพื่อเปิดรายละเอียด</em></button>{showActions && <footer className="approval-mobile-actions">{rowActions(row)}</footer>}</article>) : approvalEmptyState}</div>;
  const approvalSurface = <ResponsiveDataTable ariaLabel="รายการประวัติการอนุมัติตารางกะ" loading={loading} error={Boolean(error)} loadingLabel="กำลังอ่านประวัติการอนุมัติตาราง…" errorLabel="ไม่สามารถโหลดประวัติการอนุมัติ" className="signature-data-surface" desktop={approvalDesktop} mobile={approvalMobile} />;
...
function Dashboard() {
  ...
  const openCreateOperation = () => {
    ...
    if (activePage === 'leave') runEditor({
      ...
      fields: [{ name: 'employeeId', label: 'พนักงาน', type: 'select', required: auth.user?.role !== 'VIEWER', options: employeeOptions }, { name: 'leaveType', label: 'ประเภทการลา', type: 'select', required: true, options: leaveTypeOptions }, { name: 'startDate', label: 'วันที่เริ่ม', type: 'date', required: true }, { name: 'endDate', label: 'วันที่สิ้นสุด', type: 'date', required: true }, { name: 'substitute', label: 'ผู้เข้าเวร / ปฏิบัติงานแทน', required: true }, { name: 'reason', label: 'เหตุผล', type: 'textarea' }, { name: 'attachment', label: 'ไฟล์แนบ (ถ้ามี)', type: 'file', accept: '.pdf,.jpg,.jpeg,.png,application/pdf,image/jpeg,image/png', hint: 'ระบบปรับไฟล์อัตโนมัติ · รูปเป้าหมาย 300–450 KB (สูงสุด 500 KB) · PDF สูงสุด 1 MB' }],
      values: {}
  ...
  const handleOperationAction = async (row: DataRow, action: string) => {
    ...
    if (action === 'document' && activePage === 'licenses') {
      ...
      fields: [{ name: 'licenseNumber', label: 'เลขใบอนุญาต', required: true }, { name: 'document', label: 'ไฟล์ใบอนุญาต', type: 'file', required: true, accept: '.pdf,.jpg,.jpeg,.png,application/pdf,image/jpeg,image/png', hint: 'ระบบปรับไฟล์อัตโนมัติ · รูปเป้าหมาย 300–450 KB (สูงสุด 500 KB) · PDF สูงสุด 1 MB' }, { name: 'proposedStartDate', label: 'วันที่เริ่มต้น', type: 'date', required: true }, { name: 'proposedExpiryDate', label: 'วันหมดอายุ', type: 'date', required: true }, { name: 'note', label: 'หมายเหตุ', type: 'textarea' }],
  ...
  const content = () => {
    ...
    if (activePage === 'shiftSetup') {
      const renderShiftActions = (shiftType: DataRow, isCore: boolean, isActive: boolean) => auth.user?.role === 'ADMIN' ? <><button className="btn-secondary compact" onClick={() => openShiftTypeEditor(shiftType)}>แก้ไข</button>{isCore ? <span className="muted-text">ล็อกสถานะ</span> : <button className={isActive ? 'btn-warning compact' : 'btn-success compact'} onClick={() => toggleShiftTypeActive(shiftType)}>{isActive ? 'ปิดใช้งาน' : 'เปิดใช้งาน'}</button>}</> : null;
      const shiftDesktop = <div className="table-card shift-setup-desktop-table"><div className="table-scroll data-table-scroll"><table className="data-table data-surface-table"><thead><tr><th scope="col">Shift Code</th><th scope="col">ชื่อกะ</th><th scope="col">เวลาเริ่ม</th><th scope="col">เวลาเลิก</th><th scope="col">ชั่วโมง</th><th scope="col">สี</th><th scope="col">สถานะ</th>{auth.user?.role === 'ADMIN' && <th scope="col">จัดการ</th>}</tr></thead><tbody>{visibleShiftTypes.length ? visibleShiftTypes.map((shiftType) => { const code = String(shiftType.code || '').toUpperCase(); const isCore = ['D', 'N', 'OFF', 'AL'].includes(code); const isActive = shiftType.isActive !== false; return <tr key={text(shiftType.id)}><td><code>{text(shiftType.code)}</code>{isCore && <small className="cell-note">Core</small>}</td><td className="employee-name">{text(shiftType.name)}</td><td>{text(shiftType.startTime)}</td><td>{text(shiftType.endTime)}</td><td>{text(shiftType.hours)}</td><td><span className="shift-color" style={{ backgroundColor: String(shiftType.color || '#2F80FF') }} /> {text(shiftType.color)}</td><td><span className={`status-badge ${isActive ? 'status-badge--success' : 'status-badge--neutral'}`}>{isActive ? 'ใช้งาน' : 'ปิดใช้งาน'}</span></td>{auth.user?.role === 'ADMIN' && <td className="row-actions data-row-actions">{renderShiftActions(shiftType, isCore, isActive)}</td>}</tr>; }) : <tr><td colSpan={auth.user?.role === 'ADMIN' ? 8 : 7} className="no-rows data-table-empty-cell"><div className="data-state data-state--empty"><strong>ยังไม่มีข้อมูลรหัสกะ</strong></div></td></tr>}</tbody></table></div></div>;
      const shiftMobile = <div className="shift-setup-mobile-list" aria-label="รายการรหัสกะสำหรับมือถือ">{visibleShiftTypes.length ? visibleShiftTypes.map((shiftType) => { const code = String(shiftType.code || '').toUpperCase(); const isCore = ['D', 'N', 'OFF', 'AL'].includes(code); const isActive = shiftType.isActive !== false; return <article className="shift-setup-mobile-card data-mobile-card" key={`shift-mobile-${text(shiftType.id)}`}><header><div><code>{text(shiftType.code)}</code>{isCore && <small>Core</small>}</div><span className={`status-badge ${isActive ? 'status-badge--success' : 'status-badge--neutral'}`}>{isActive ? 'ใช้งาน' : 'ปิดใช้งาน'}</span></header><h2>{text(shiftType.name)}</h2><dl><div><dt>เวลา</dt><dd>{text(shiftType.startTime)}–{text(shiftType.endTime)}</dd></div><div><dt>ชั่วโมง</dt><dd>{text(shiftType.hours)}</dd></div><div><dt>สี</dt><dd><span className="shift-color" style={{ backgroundColor: String(shiftType.color || '#2F80FF') }} /> {text(shiftType.color)}</dd></div></dl>{auth.user?.role === 'ADMIN' && <footer>{renderShiftActions(shiftType, isCore, isActive)}</footer>}</article>; }) : <div className="data-state data-state--empty"><strong>ยังไม่มีข้อมูลรหัสกะ</strong></div>}</div>;
      return (
        <section className="view-pane">
          ...
          <ResponsiveDataTable ariaLabel="รายการรหัสกะ" hasRows={visibleShiftTypes.length > 0} className="shift-setup-responsive-table" desktop={shiftDesktop} mobile={shiftMobile} />
        </section>
      );
    }
    if (activePage === 'schedule') {
    ...
    if (['leave', 'leavePending', 'leaveHistory'].includes(activePage)) {
      ...
      const remaining = nested(leaveSummary.remaining);
        ...
        return <LeaveManagementPage mode={activePage === 'leavePending' ? 'pending' : activePage === 'leaveHistory' ? 'history' : 'all'} historyScope={activePage === 'leaveHistory' ? 'all' : 'mine'} historyMonth={activePage === 'leaveHistory' ? leaveMonth : undefined} historyTotal={activePage === 'leaveHistory' ? operationResponse.meta?.total : undefined} historyPage={activePage === 'leaveHistory' ? operationResponse.meta?.page : undefined} historyTotalPages={activePage === 'leaveHistory' ? operationResponse.meta?.totalPages : undefined} historyStatusCounts={activePage === 'leaveHistory' ? operationResponse.meta?.statusCounts : undefined} employeeId={String(leaveSummary.employeeId || '')} currentUserId={auth.user?.id} currentUserRole={auth.user?.role} leavePolicy={leavePolicy} leaveTypes={activeLeaveTypes} rows={rows} loading={operationLoading} error={operationError} linked={Boolean(leaveSummary.linked)} remaining={remaining} quotaYear={Number(leaveSummary.quotaYear || currentBangkokQuotaYear())} canManage={pwaShell ? false : canManage} canSubmit={(pwaShell ? pwaOnline : true) && (auth.user?.role !== 'VIEWER' || Boolean(leaveSummary.linked))} canCancelApprovedLeave={canCancelApprovedLeave} mutationsEnabled={!pwaShell || pwaOnline} employeeOptions={employeeOptions} onRefresh={() => setOperationRefresh((value) => value + 1)} onHistoryMonthChange={changeLeaveMonth} onHistoryMonthStep={(delta) => changeLeaveMonth(shiftMonthValue(leaveMonth, delta))} onHistoryPageChange={setOperationPage} onApprove={(row) => handleOperationAction(row, 'approve')} onReject={(row) => handleOperationAction(row, 'reject')} onReturnForCorrection={(row) => handleOperationAction(row, 'return')} onEditReturned={(row) => runEditor({
          ...
          fields: [
            { name: 'leaveType', label: 'ประเภทการลา', type: 'select', required: true, options: leaveTypeOptionsForRow(row) },
    ...
    <>
      ...
      <main className="main-area">
        ...
        {pwaShell && <nav className="pwa-bottom-nav" aria-label="เมนู PWA">
          ...
          <button type="button" className={activePage === 'employeeSchedule' ? 'active' : ''} onClick={() => selectPwaPage('employeeSchedule')}><SmsIcon name="calendar" size={20} /><span>ตารางงาน</span></button>
