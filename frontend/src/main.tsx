...
function ShiftEditorModal({ shift, defaults, employees, shiftTypes, licenses, isAdmin, onClose, onSubmit }: ShiftEditorModalProps) {
  ...
  const isShiftBlocked = ['EXPIRED', 'MISSING', 'INVALID'].includes(shiftLicStatus) || String(shift?.remark || '').toLowerCase().includes('license block') || String(shift?.remark || '').includes('ใบอนุญาตไม่ผ่าน');
