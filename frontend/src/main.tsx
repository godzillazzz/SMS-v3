...
function Dashboard() {
  ...
  if (['EXPIRED', 'MISSING', 'INVALID'].includes(licStatus) || remarkStr.includes('License blocked') || remarkStr.includes('ใบอนุญาตไม่ผ่าน')) {
    return <small className="shift-note" style={{ color: '#ef4444', fontWeight: 700 }}>License Block ⛔</small>;
  }
  if (licStatus === 'OVERRIDDEN') {
    return <small className="shift-note" style={{ color: '#2563eb', fontWeight: 700 }}>Override ⚡</small>;
  }
  if (Boolean(shift.locked)) {
    return <small className="shift-note" style={{ color: '#d97706', fontWeight: 700 }}>MANUAL 🔒</small>;
  }
  return null;
