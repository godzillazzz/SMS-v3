...
describe('Attendance UX V4 Employee Mobile/PWA shell', () => {
  it('limits the installed employee product surface to the approved five self-service pages', () => {
    ...
    for (const page of ['attendance', 'attendanceHistory', 'employeeSchedule', 'leave', 'profile']) {
      expect(main).toContain(`selectPwaPage('${page}'`);
    }
  ...
  it('starts standalone PWA on Attendance and fail-closes navigation outside the five-page employee shell', () => {
  ...
  it('keeps normal web navigation intact while the PWA uses dedicated page-aware mobile chrome', () => {
    ...
    expect(main).toContain("pwa-shell pwa-page-${activePage}");
  ...
  it('ships install metadata and shortcuts for exactly the five approved employee pages', () => {
  ...
  it('registers service worker only in secure-capable browsers and never caches API data', () => {
