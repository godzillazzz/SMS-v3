...
describe('Attendance UX V4 visual acceptance contract', () => {
  ...
  it('fails closed on Attendance token refresh for View As or stale principals', () => {
    expect(authRequest).toContain('let attendanceTokenRefreshGuard');
    expect(authRequest).toContain('setAttendanceTokenRefreshGuard');
    expect(authRequest).toContain('attendanceTokenRefreshGuard(token) !== true');
    expect(main).toContain('const primaryTokenRef = useRef<string>()');
    expect(main).toContain('const viewAsTokenRef = useRef<string>()');
    expect(main).toContain('requestToken === viewAsTokenRef.current');
    expect(main).toContain('requestToken === primaryTokenRef.current');
    expect(main).toContain('setAttendanceTokenRefreshGuard(null)');
  });
