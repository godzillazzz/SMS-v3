...
const impactLabels: Record<keyof Preflight['impacts'], string> = { futureShiftAssignments: 'เวรในอนาคต', pendingLeaveRequests: 'ลารอพิจารณา', approvedFutureLeaveRequests: 'ลาอนุมัติในอนาคต', activeLicenses: 'ใบอนุญาต', licenseDocuments: 'เอกสารใบอนุญาต' };
const approvedFutureRequest = (requests: ChangeRequest[]) => requests.find((request) => request.status === 'APPROVED' && !request.appliedAt && request.revisions?.some((revision) => revision.effectiveMode === 'FUTURE_EFFECTIVE'));
...
export function EmployeeGovernedEditModal({ token, employee, role, onClose, onChanged }: Props) {
  ...
  const futureApprovedRequest = useMemo(() => approvedFutureRequest(requests), [requests]);
  const impactGroups = useMemo(() => {
    if (!preflight) return { noImpact: [] as string[], review: [] as string[], followUp: [] as string[] };
    const impactSnapshot: Preflight['impacts'] = { futureShiftAssignments: preflight.impacts.futureShiftAssignments, pendingLeaveRequests: preflight.impacts.pendingLeaveRequests, approvedFutureLeaveRequests: preflight.impacts.approvedFutureLeaveRequests, activeLicenses: preflight.impacts.activeLicenses, licenseDocuments: preflight.impacts.licenseDocuments };
    const followUp = (Object.entries(impactSnapshot) as Array<[keyof Preflight['impacts'], number | undefined]>).filter(([, count]) => Number(count || 0) > 0).map(([key, count]) => `${impactLabels[key]} ${count}`);
    const review = preflight.warnings.map((warning) => warning.count !== undefined ? `${warning.message} (${warning.count})` : warning.message);
    const noImpact = !followUp.length && !review.length ? ['ไม่พบรายการที่ต้องตรวจสอบหรือติดตามเพิ่มเติม'] : [];
    return { noImpact, review, followUp };
  }, [preflight]);
  ...
  const changedFieldSummary = useMemo(() => Object.keys(changes).map((field) => labels[field] || field), [changes]);
  ...
  const modal = <div className="employee-governed-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose(); }}>
    <section ref={modalRef} className="employee-governed-modal" role="dialog" aria-modal="true" aria-labelledby="employee-governed-title">
      ...
      <div className="employee-governed-body">
        ...
        {(activeRequest || futureApprovedRequest) && <section className="employee-governed-section employee-change-conflict-summary" aria-label="สถานะคำขอและรายการรอมีผล"><h3>สถานะการเปลี่ยนแปลงที่ต้องเห็นก่อนแก้ไข</h3>{activeRequest && <div className={`employee-request-banner employee-request-banner--${activeRequest.status.toLowerCase()}`}><strong>{statusLabel[activeRequest.status] || activeRequest.status}</strong><span>มีคำขอที่ยังอยู่ระหว่างดำเนินการ · Revision {activeRequest.currentRevision || '—'}</span></div>}{futureApprovedRequest && <div className="employee-request-banner employee-request-banner--approved"><strong>อนุมัติแล้ว · รอวันที่มีผล</strong><span>{futureApprovedRequest.revisions?.find((revision) => revision.effectiveMode === 'FUTURE_EFFECTIVE')?.effectiveDate ? `มีผล ${cleanDate(futureApprovedRequest.revisions?.find((revision) => revision.effectiveMode === 'FUTURE_EFFECTIVE')?.effectiveDate)}` : 'มี Future-effective change ที่ยังไม่ถูก apply'}</span></div>}<small>ระบบไม่รวมรายการเหล่านี้เข้ากับการแก้ไขใหม่โดยอัตโนมัติ และ backend ยังคงเป็นผู้ตัดสิน conflict/concurrency</small></section>}
        ...
        {isAdmin && preflight && <section className="employee-governed-section employee-impact"><h3>Impact Preview ก่อนบันทึก</h3><div className="employee-impact-groups"><article className="employee-impact-group employee-impact-group--clear"><strong>ไม่กระทบ</strong>{impactGroups.noImpact.length ? impactGroups.noImpact.map((item) => <span key={item}>{item}</span>) : <span>—</span>}</article><article className="employee-impact-group employee-impact-group--review"><strong>ต้องตรวจสอบ</strong>{impactGroups.review.length ? impactGroups.review.map((item) => <span key={item}>{item}</span>) : <span>ไม่มีคำเตือนจาก preflight</span>}</article><article className="employee-impact-group employee-impact-group--follow-up"><strong>ต้องติดตาม</strong>{impactGroups.followUp.length ? impactGroups.followUp.map((item) => <span key={item}>{item}</span>) : <span>ไม่มีรายการอ้างอิงที่ได้รับผลกระทบ</span>}</article></div><small className="employee-impact-authority-note">การจัดกลุ่มนี้เป็นการนำเสนอผลจาก authoritative preflight เท่านั้น ไม่ได้สร้างกฎอนุมัติใหม่บน client</small>{preflight.warnings.length > 0 && <label className="employee-warning-confirm"><input type="checkbox" checked={acknowledgeWarnings} onChange={(event) => setAcknowledgeWarnings(event.target.checked)} /> ตรวจสอบคำเตือนแล้วและยืนยันดำเนินการ</label>}</section>}
