  ...
  return <div className="personnel-table-card data-surface-card">
    <div className="personnel-table-scroll data-table-scroll">
      <table className="personnel-table data-surface-table">
        <thead><tr><th scope="col">พนักงาน</th><th scope="col">รหัสภายใน</th><th scope="col">หน่วยงาน</th><th scope="col">ตำแหน่ง</th><th scope="col">สถานะ</th>{canManage && <th scope="col">จัดการ</th>}</tr></thead>
        ...
        >
          ...
          {canManage && <td className="personnel-row-actions data-row-actions" onClick={(event) => event.stopPropagation()}><button type="button" className="data-row-primary-action" aria-label={`แก้ไขข้อมูล ${employee.firstName} ${employee.lastName}`} onClick={() => onEdit(employee)}>แก้ไขข้อมูล</button>{role === 'ADMIN' && <DataRowActionMenu label={`การทำงานเพิ่มเติมสำหรับ ${employee.firstName} ${employee.lastName}`} actions={[{ label: 'จัดการสถานะพนักงาน', onSelect: () => onLifecycle(employee) }]} />}</td>}
