    ...
    <section className="employee-review-modal" role="dialog" aria-modal="true" aria-labelledby="employee-review-title">
      ...
      <div className="employee-review-layout">
        ...
        <div className="employee-review-detail">{error && <div className="employee-governed-alert employee-governed-alert--error"><RequestErrorContent error={error} /></div>}{selected && revision ? <>
          <section className="employee-review-summary"><div><small>พนักงาน</small><strong>{selected.employee?.firstName} {selected.employee?.lastName}</strong><span>{selected.employee?.employeeCode} · {selected.employee?.department || 'ไม่ระบุหน่วยงาน'}</span></div><div><small>ผู้ส่งคำขอ</small><strong>{selected.requestOwner?.displayName || 'Manager'}</strong><span>{roleDisplayName(selected.requestOwnerRoleSnapshot || selected.requestOwner?.role || 'MANAGER')} · Revision {revision.revision}</span><span>{new Intl.DateTimeFormat('th-TH', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(revision.submittedAt))}</span></div><div><small>สถานะ / วันที่มีผล</small><strong>{selected.status}</strong><span>{revision.effectiveMode === 'FUTURE_EFFECTIVE' ? `FUTURE_EFFECTIVE · ${String(revision.effectiveDate || '').slice(0, 10)}` : 'IMMEDIATE · เมื่ออนุมัติสำเร็จ'}</span></div></section>
