...
function MasterColumn({ title, rows, kind, busy, onCreate, onEdit, onActivate, onPreflight, onDeactivate }: ColumnProps) {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      setEditError(formatRequestErrorMessage(error, 'บันทึก Master ไม่สำเร็จ'));
    }
  };
...
export function PersonnelMasterPanel({ token }: { token: string }) {
  ...
  const create = async (kind: MasterKind, code: string, name: string) => { setBusy('create-' + kind); setNotice(''); try { await api.createPersonnelMaster(token, kind, { code, name, isActive: true }); await load(); setNotice('เพิ่ม ' + (kind === 'department' ? 'Department' : 'Position') + ' Master แล้ว'); } catch (error) { setNotice(formatRequestErrorMessage(error, 'เพิ่ม Master ไม่สำเร็จ')); } finally { setBusy(''); } };
  const edit = async (kind: MasterKind, row: MasterRow, input: Pick<MasterRow, 'name' | 'sortOrder'>) => { setBusy(row.id); setNotice(''); try { await api.updatePersonnelMaster(token, kind, row.id, input); await load(); setNotice(row.name + ': บันทึกการแก้ไขแล้ว'); } catch (error) { const message = formatRequestErrorMessage(error, 'บันทึก Master ไม่สำเร็จ'); setNotice(message); throw error; } finally { setBusy(''); } };
  const activate = async (kind: MasterKind, row: MasterRow) => { setBusy(row.id); setNotice(''); try { await api.updatePersonnelMaster(token, kind, row.id, { isActive: true }); await load(); setNotice(row.name + ': เปิดใช้งานแล้ว'); } catch (error) { setNotice(formatRequestErrorMessage(error, 'บันทึก Master ไม่สำเร็จ')); } finally { setBusy(''); } };
  const preflight = async (kind: MasterKind, row: MasterRow) => { setBusy(row.id); setNotice(''); try { const result=await api.personnelMasterImpact(token,kind,row.id); return result?.data as MasterImpact; } catch(error) { setNotice(formatRequestErrorMessage(error, 'ตรวจสอบผลกระทบไม่สำเร็จ')); throw error; } finally { setBusy(''); } };
  const deactivate = async (kind: MasterKind, row: MasterRow, reason: string) => { setBusy(row.id); setNotice(''); try { await api.updatePersonnelMaster(token, kind, row.id, { isActive: false, confirmImpact: true, reason }); await load(); setNotice(row.name + ': ปิดใช้งานแล้ว'); } catch(error) { setNotice(formatRequestErrorMessage(error, 'ปิดใช้งาน Master ไม่สำเร็จ')); throw error; } finally { setBusy(''); } };
