...
type WorkforceOverviewCardProps = { totalEmployees: number; activeEmployees: number; monthShifts: number; loading: boolean; onNavigate?: DashboardNavigate };

export function WorkforceOverviewCard({ totalEmployees, activeEmployees, monthShifts, loading, onNavigate }: WorkforceOverviewCardProps) {
  ...
  return <section className="dashboard-panel dashboard-workforce">
    ...
    {loading ? <div className="dashboard-panel__loading"><span className="dashboard-skeleton dashboard-skeleton--donut" /><span className="dashboard-skeleton dashboard-skeleton--lines" /></div> : totalEmployees === 0 ? <div className="dashboard-workforce__empty"><span className="dashboard-workforce__empty-icon">◌</span><strong>ยังไม่มีข้อมูลกำลังคน</strong><p>ระบบยังไม่พบข้อมูลบุคลากรที่สามารถนำมาสรุปได้</p>{onNavigate && <button type="button" onClick={() => onNavigate('employees')}>ไปที่ Personnel Directory</button>}</div> : <div className="dashboard-workforce__body">
      ...
      <div className="dashboard-legend"><div><span className="dashboard-legend__dot dashboard-legend__dot--indigo" /><p>พนักงานทั้งหมด</p><b>{formatMetric(totalEmployees)}</b></div><div><span className="dashboard-legend__dot dashboard-legend__dot--green" /><p>พร้อมปฏิบัติงาน</p><b>{formatMetric(activeEmployees)}</b></div><div><span className="dashboard-legend__dot dashboard-legend__dot--teal" /><p>กะในเดือนนี้</p><b>{formatMetric(monthShifts)}</b></div></div>
    </div>}
