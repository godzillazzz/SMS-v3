...
export function AttentionNeededCard({ rows, expiringLicenses, loading, onNavigate }: AttentionNeededCardProps) {
  ...
  return <section className="dashboard-panel dashboard-attention" aria-label="Action Center">
    <header className="dashboard-panel__header"><div><p>ACTION CENTER</p><h2>ศูนย์ดำเนินการ</h2></div><span className={'dashboard-attention__count ' + (visibleRows.length ? '' : 'is-clear')}>{visibleRows.length ? formatMetric(visibleRows.length) + ' กลุ่มงาน' : 'ปกติดี'}</span></header>
