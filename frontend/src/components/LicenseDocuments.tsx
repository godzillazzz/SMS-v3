...
import { activateTabFromKeyboard } from './AccessibleTabs';
import {
  formatFileSize,
  formatLicenseDate,
  ...
  return createPortal(
    <div className="license-modal-viewport" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className={`license-modal-dialog ${className}`.trim()} role="dialog" aria-modal="true" aria-labelledby={labelledBy} onMouseDown={(event) => event.stopPropagation()}>
        <button ref={closeRef} type="button" className="license-modal-close" aria-label="ปิดหน้าต่าง" onClick={onClose}><SmsIcon name="close" size={18} /></button>
        {children}
      </section>
    </div>,
...
export function LicenseDocumentViewerModal({ documentId, onRequestView, onClose }: { documentId: string; onRequestView: (id: string) => Promise<ViewResult>; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    setViewer(undefined);
    setError(undefined);
    onRequestView(documentId).then((result) => { if (active) setViewer(result); }).catch((reason) => { if (active) setError(licenseRequestError(reason)); });
...
export function LicenseTableDocumentColumns({ license, summary, services, isAdmin, onChanged }: { license: LicenseIdentity; summary?: LicenseTableDocumentSummary; services: Services; isAdmin: boolean; onChanged: (message: string) => void }) {
  ...
  const [reviewError, setReviewError] = useState<RequestErrorInput>();
  ...
  const openReview = async () => {
    ...
    } catch (reason) { setReviewError(licenseRequestError(reason)); }
...
function ReviewModal({ document, license, services, isAdmin, onClose, onChanged }: { document: LicenseDocument; license: LicenseIdentity; services: Services; isAdmin: boolean; onClose: () => void; onChanged: (message: string) => Promise<void> }) {
  ...
  return <>
    <ModalFrame labelledBy="license-review-title" className="license-review-dialog" onClose={onClose} escapeEnabled={!viewerId}>
      ...
      <button type="button" className="btn-ghost license-view-button" aria-label="ดูไฟล์ใบอนุญาต" onClick={() => setViewerId(document.id)}><SmsIcon name="eye" size={16} /> ดูไฟล์</button>
  ...
  return <>
    <ModalFrame labelledBy="license-history-title" className="license-history-dialog" onClose={onClose} escapeEnabled={!viewerId}>
      ...
      <div className="license-history-list">{sorted.map((document) => <article key={document.id} className="license-history-card">
        ...
        <div className="license-inline-actions">{document.fileAvailable !== false && <button type="button" className="btn-ghost compact" aria-label="ดูไฟล์ใบอนุญาต" onClick={() => setViewerId(document.id)}><SmsIcon name="eye" size={16} /> ดูไฟล์</button>}{isAdmin && canPermanentlyDeleteDocument(document, documents) && <button type="button" className="btn-danger-outline compact" aria-label="ลบเอกสารใบอนุญาตถาวร" onClick={() => setDeleteDocument(document)}>ลบถาวร</button>}</div>
      </article>)}</div>
    </ModalFrame>
    {viewerId && <LicenseDocumentViewerModal documentId={viewerId} onRequestView={services.view} onClose={() => setViewerId(undefined)} />}
...
function CorrectionModal({ document, services, onClose, onChanged }: { document: LicenseDocument; services: Services; onClose: () => void; onChanged: (message: string) => Promise<void> }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (file && file.size > ATTACHMENT_POLICIES.DOCUMENT.maxSourceBytes) { setError('ไฟล์ต้นฉบับมีขนาดใหญ่เกิน 20 MB'); return; }
  ...
  return <ModalFrame labelledBy="license-correction-title" className="license-review-dialog" onClose={onClose}>
    ...
    <form className="license-upload-form" onSubmit={submit}>
      ...
      <label className="field-group"><span>ไฟล์ใหม่ (ไม่บังคับ)</span><input type="file" accept=".pdf,.jpg,.jpeg,.png,application/pdf,image/jpeg,image/png" onChange={(event) => setFile(event.target.files?.[0])} /><small>{file ? 'ไฟล์ใหม่จะเป็น Revision ถัดไป โดยเก็บไฟล์ Revision เดิมไว้ในประวัติ' : 'ใช้ไฟล์เดิมต่อได้ · ไฟล์ใหม่จะปรับขนาดอัตโนมัติ: รูป ≤ 500 KB, PDF ≤ 1 MB'}</small></label>
...
export function LicenseEditModal({ license, isAdmin, currentUserId, services, onUpload, onChanged, onClose }: { license: LicenseIdentity; isAdmin: boolean; currentUserId: string; services: Services; onUpload: (data: UploadData, file: File) => Promise<void>; onChanged: (message: string) => void; onClose: () => void }) {
  ...
  const load = async () => { try { setDocuments(sortLicenseDocuments(await services.list(license.id))); } catch (reason) { setError(licenseRequestError(reason)); } };
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (file.size > ATTACHMENT_POLICIES.DOCUMENT.maxSourceBytes) { setError('ไฟล์ต้นฉบับมีขนาดใหญ่เกิน 20 MB'); return; }
    if (!licenseNumber.trim()) { setError('กรุณากรอกเลขใบอนุญาต'); return; }
  ...
  const documentLine = (document: LicenseDocument) => <div className="license-edit-document" key={document.id}>
    ...
    <div className="license-inline-actions">
      {document.fileAvailable !== false && <button type="button" className="btn-icon-only" aria-label="ดูใบอนุญาต" title="ดูใบอนุญาต" onClick={() => setViewerId(document.id)}><SmsIcon name="eye" size={16} /></button>}
  ...
  return <>
    <ModalFrame labelledBy="license-edit-title" className="license-edit-dialog" onClose={onClose} escapeEnabled={!reviewDocument && !correctionDocument && !viewerId}>
      ...
      <div className="license-edit-tabs" role="tablist" aria-label="ส่วนข้อมูลใบอนุญาต" onKeyDown={activateTabFromKeyboard}><button id="license-tab-details" type="button" role="tab" aria-selected={tab === 'details'} aria-controls="license-panel-details" tabIndex={tab === 'details' ? 0 : -1} className={tab === 'details' ? 'is-active' : ''} onClick={() => setTab('details')}>ข้อมูลใบอนุญาต</button><button id="license-tab-documents" type="button" role="tab" aria-selected={tab === 'documents'} aria-controls="license-panel-documents" tabIndex={tab === 'documents' ? 0 : -1} className={tab === 'documents' ? 'is-active' : ''} onClick={() => setTab('documents')}>เอกสารและประวัติการดำเนินการ</button></div>
      {tab === 'details' && <div id="license-panel-details" role="tabpanel" aria-labelledby="license-tab-details"><dl className="license-document-facts"><div><dt>เลขใบอนุญาตปัจจุบัน</dt><dd>{license.licenseNumber || 'ยังไม่มีเลขใบอนุญาต'}</dd></div><div><dt>ประเภทใบอนุญาต</dt><dd>{license.licenseType || '-'}</dd></div><div><dt>วันที่เริ่มต้นปัจจุบัน</dt><dd>{formatLicenseDate(license.issueDate)}</dd></div><div><dt>วันหมดอายุปัจจุบัน</dt><dd>{formatLicenseDate(license.expiryDate)}</dd></div><div><dt>สถานะ</dt><dd>{license.status || '-'}</dd></div><div><dt>หน่วยงาน</dt><dd>{license.employee.department || '-'}</dd></div></dl></div>}
      {tab === 'documents' && <div id="license-panel-documents" role="tabpanel" aria-labelledby="license-tab-documents" className="license-edit-documents"><div className="license-edit-current"><h3>เอกสารปัจจุบัน</h3>{summary.current ? documentLine(summary.current) : <p>ยังไม่มีเอกสารที่อนุมัติแล้ว</p>}{summary.pending.length > 0 && <><h3>เอกสารต่ออายุที่รอตรวจสอบ</h3>{summary.pending.map(documentLine)}</>}{summary.returned.length > 0 && <><h3>เอกสารที่ส่งกลับแก้ไข</h3>{summary.returned.map(documentLine)}</>}{documents.length > 0 && <button type="button" className="btn-info-outline compact" onClick={() => setHistoryOpen(true)}>ดูประวัติการดำเนินการ</button>}</div><form className="license-upload-form" onSubmit={submit}><h3>แนบใบอนุญาตใหม่</h3><label className="field-group"><span>เลขใบอนุญาต <b>*</b></span><input value={licenseNumber} maxLength={100} onChange={(event) => setLicenseNumber(event.target.value)} required /></label><label className="field-group"><span>ไฟล์ใบอนุญาต <b>*</b></span><input type="file" accept=".pdf,.jpg,.jpeg,.png,application/pdf,image/jpeg,image/png" onChange={(event) => setFile(event.target.files?.[0])} required /><small>ระบบปรับขนาดอัตโนมัติ · รูปเป้าหมาย 300–450 KB (สูงสุด 500 KB) · PDF สูงสุด 1 MB</small></label><div className="license-date-comparison"><label className="field-group"><span>วันที่เริ่มต้น <b>*</b></span><input type="date" value={proposedStartDate} onChange={(event) => setProposedStartDate(event.target.value)} required /></label><label className="field-group"><span>วันหมดอายุ <b>*</b></span><input type="date" value={proposedExpiryDate} onChange={(event) => setProposedExpiryDate(event.target.value)} required /></label></div><label className="field-group"><span>หมายเหตุ</span><textarea rows={3} value={note} onChange={(event) => setNote(event.target.value)} maxLength={2000} /></label>{error && <div className="license-modal-error" role="alert"><RequestErrorContent error={error} /></div>}<button type="submit" className="btn-primary" disabled={busy}>{busy ? 'กำลังส่ง…' : 'ส่งตรวจสอบ'}</button></form></div>}
    </ModalFrame>
...
export function LicenseDocumentsCell({ license, isAdmin, refreshSignal, services, onUpload, onChanged, onEdit }: { license: LicenseIdentity; isAdmin: boolean; refreshSignal: number; services: Services; onUpload: () => void; onChanged: (message: string) => void; onEdit?: () => void }) {
  ...
  const documentLine = (document: LicenseDocument, renewal = false) => <div className="license-document-line" key={document.id}>
    ...
    <div className="license-inline-actions">{document.fileAvailable !== false && <button type="button" className="btn-icon-only" aria-label="ดูไฟล์ใบอนุญาต" title="ดูไฟล์ใบอนุญาต" onClick={() => setViewerId(document.id)}><SmsIcon name="eye" size={16} /></button>}{isAdmin && document.status === 'PENDING' && <button type="button" className="btn-warning compact" onClick={() => setReviewDocument(document)}>ตรวจสอบ</button>}</div>
  </div>;
  return <div className="license-documents-cell">
    {loading && <span className="license-doc-loading" role="status">กำลังอ่านเอกสาร…</span>}
    {error && <span className="license-doc-inline-error" role="alert"><RequestErrorContent error={error} /></span>}
