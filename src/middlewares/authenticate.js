...
async function authenticate(req, _res, next) {
  ...
  try {
    ...
    const user = await prisma.user.findUnique({ where: { id: claims.sub }, select: { id: true, email: true, role: true, isActive: true, tokenVersion: true } });
    if (!user || !user.isActive || user.tokenVersion !== claims.tokenVersion) throw new Error('Token no longer valid.');
