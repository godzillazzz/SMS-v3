...
function createAttendanceSupervisorService({ prisma = prismaDefault, clock = () => new Date(), siteAuthorityService = null } = {}) {
  ...
  async function detail({ actor, assignmentId } = {}, client = prisma) {
    ...
    const faceSessionIds = [...new Set(rawEvents.map((event) => event.faceVerificationSessionId).filter(Boolean))];
    const evidenceRows = faceSessionIds.length && client.attendanceEvidence
      ? await client.attendanceEvidence.findMany({
        where: { faceVerificationSessionId: { in: faceSessionIds } },
        select: { id: true, faceVerificationSessionId: true, capturedAt: true, retentionUntil: true, purgedAt: true }
      })
      : [];
    const evidenceBySession = new Map(evidenceRows.map((evidence) => [evidence.faceVerificationSessionId, evidence]));
    ...
    return {
      ...
      rawEvents: rawEvents.map((event) => ({
        ...
        evidence: (() => {
          const evidence = evidenceBySession.get(event.faceVerificationSessionId);
          return evidence ? { id: evidence.id, capturedAt: evidence.capturedAt, retentionUntil: evidence.retentionUntil, purgedAt: evidence.purgedAt } : null;
        })()
      })),
      governance: {
