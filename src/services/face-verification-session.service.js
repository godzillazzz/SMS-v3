  ...
  async function createSession({ actor, purpose, contextDigest }) {
    ...
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        return { session: safeSession(session), challengeId: deviceChallenge.id, challenge, keyAlgorithm: binding.device.keyAlgorithm };
      });
  ...
  async function recordTrustedProviderResult({ sessionId, providerSessionRef, padPassed, faceMatchPassed, injectionRiskDetected = false, resultCode = null, policyProfileId = null, engineVersion = null }) {
    ...
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        const binding = await exactActiveBinding(tx, session.employeeId);
        if (binding.device.id !== session.deviceEnrollmentId || binding.referencePhoto.id !== session.referencePhotoId) throw http(409, 'VERIFICATION_STALE', 'Device or Reference Photo authority changed during verification.');
