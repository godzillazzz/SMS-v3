...
async function list(query, role) {
  ...
  return { data: role === 'USER' ? employees.map(publicEmployee) : employees, meta: { page, pageSize, total, totalPages: Math.ceil(total / pageSize) } };
}
async function getById(id, role) { const employee = await prisma.employee.findFirst({ where: { id, deletedAt: null } }); if (!employee) throw new HttpError(404, 'Employee not found.'); return role === 'USER' ? publicEmployee(employee) : employee; }
