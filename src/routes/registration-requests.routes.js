...
router.post('/:id/match', async (req, res, next) => {
  try { res.json({ data: await service.match({ id: uuid.parse(req.params.id), employeeId: matchSchema.parse(req.body).employeeId, actorUserId: req.user.sub }) }); } catch (error) { next(error); }
});
...
router.post('/:id/reject', async (req, res, next) => {
  try { const input = rejectSchema.parse(req.body); res.json({ data: await service.reject({ id: uuid.parse(req.params.id), reason: input.reason, actorUserId: req.user.sub }) }); } catch (error) { next(error); }
});
