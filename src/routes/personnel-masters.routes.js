...
router.get('/', authorize('ADMIN', 'MANAGER', 'SUPERVISOR'), async (req, res, next) => { try { res.json({ data: await service.list({ activeOnly: req.query.activeOnly === 'true' }) }); } catch (error) { next(error); } });
