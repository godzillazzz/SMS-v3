...
router.post('/', authorize('ADMIN', 'HR'), async (req, res, next) => { try { res.status(201).json({ data: await employee.create(employeeSchema.parse(req.body), req.user.sub) }); } catch (error) { next(error); } });
router.put('/:id', authorize('ADMIN', 'HR'), async (req, res, next) => { try { const body = employeeSchema.partial().parse(req.body); if (Object.keys(body).length === 0) throw new HttpError(400, 'Update body cannot be empty.'); res.json({ data: await employee.update(uuid.parse(req.params.id), body, req.user.sub) }); } catch (error) { next(error); } });
