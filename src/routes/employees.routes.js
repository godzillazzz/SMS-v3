...
router.get('/readiness/center', authorize('ADMIN', 'MANAGER', 'SUPERVISOR'), async (req, res, next) => { try { const query = z.object({ search: z.string().trim().max(100).optional(), limit: z.coerce.number().int().min(1).max(50).default(50) }).parse(req.query); res.json(await onboardingReadiness.listEmployeeReadiness(query)); } catch (error) { next(error); } });
router.get('/:id/onboarding-readiness', authorize('ADMIN', 'MANAGER', 'SUPERVISOR'), async (req, res, next) => { try { res.json({ data: await onboardingReadiness.getEmployeeReadiness({ employeeId: uuid.parse(req.params.id) }) }); } catch (error) { next(error); } });
router.get('/:id/lifecycle', authorize('ADMIN', 'MANAGER', 'SUPERVISOR'), async (req, res, next) => { try { const query = z.object({ page: z.coerce.number().int().min(1).default(1), pageSize: z.coerce.number().int().min(1).max(100).default(25) }).parse(req.query); res.json(await lifecycle.getEmployeeLifecycleHistory(uuid.parse(req.params.id), query, req.user.role)); } catch (error) { next(error); } });
router.get('/:id/lifecycle/state', authorize('ADMIN', 'MANAGER', 'SUPERVISOR'), async (req, res, next) => { try { const query = z.object({ date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/) }).parse(req.query); res.json({ data: await lifecycle.getEmployeeStateAt(uuid.parse(req.params.id), query.date) }); } catch (error) { next(error); } });
...
router.get('/:id/change-requests', authorize('ADMIN', 'MANAGER', 'SUPERVISOR'), async (req, res, next) => {
...
router.post('/:id/change-requests', authorize('MANAGER', 'SUPERVISOR'), async (req, res, next) => {
...
router.post('/', authorize('ADMIN', 'MANAGER', 'SUPERVISOR'), async (req, res, next) => { try { res.status(201).json({ data: await employee.create(employeeSchema.parse(req.body), req.user.sub) }); } catch (error) { next(error); } });
router.put('/:id', authorize('ADMIN', 'MANAGER', 'SUPERVISOR'), async (req, res, next) => {
  try {
    if (['MANAGER', 'SUPERVISOR'].includes(req.user.role)) throw new HttpError(403, 'Manager Employee edits require a governed change request.', { code: 'EMPLOYEE_CHANGE_REQUEST_REQUIRED' });
