...
test('Q13C ADMIN: Security Site reversible configuration workflow', async () => {
  ...
  const created = await authenticatedRequest('/api/v1/security-sites', { accessToken, method: 'POST', data: { code: SITE_CODE, name: SITE_NAME, latitude: 13.7563, longitude: 100.5018, geofenceRadiusMeters: 25, isActive: true } });
  expect(created.status).toBe(201);
  const siteId = created.payload?.data?.id;
  expect(siteId).toBeTruthy();
  const updated = await authenticatedRequest(`/api/v1/security-sites/${siteId}`, { accessToken, method: 'PUT', data: { name: `${SITE_NAME} Updated`, geofenceRadiusMeters: 30 } });
  expect(updated.status).toBe(200);
  expect(updated.payload?.data?.geofenceRadiusMeters).toBe(30);
  const mapped = await authenticatedRequest('/api/v1/security-sites/department-mapping', { accessToken, method: 'PUT', data: { departmentMasterId: department.id, siteIds: [siteId], defaultSiteId: siteId } });
  expect(mapped.status).toBe(200);
  expect(mapped.payload?.data?.defaultSiteId).toBe(siteId);
  const rotated = await authenticatedRequest(`/api/v1/security-sites/${siteId}/qr/rotate`, { accessToken, method: 'POST', data: { reason: `${MARKER} QR rotate contract` } });
  expect(rotated.status).toBe(200);
  const credentialId = rotated.payload?.data?.credential?.id;
  expect(credentialId).toBeTruthy();
  const revoked = await authenticatedRequest(`/api/v1/security-sites/${siteId}/qr/${credentialId}/revoke`, { accessToken, method: 'POST', data: { reason: `${MARKER} QR revoke contract` } });
  expect(revoked.status).toBe(200);
  expect(revoked.payload?.data?.revokedAt).toBeTruthy();
  const unmapped = await authenticatedRequest('/api/v1/security-sites/department-mapping', { accessToken, method: 'PUT', data: { departmentMasterId: department.id, siteIds: [], defaultSiteId: null } });
  expect(unmapped.status).toBe(200);
  expect(unmapped.payload?.data?.siteIds || []).toHaveLength(0);
  const deactivated = await authenticatedRequest(`/api/v1/security-sites/${siteId}`, { accessToken, method: 'PUT', data: { isActive: false, reason: `${MARKER} reversible site deactivation` } });
