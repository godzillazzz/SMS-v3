...
The browser sends `clientType: "browser"`. Login and refresh return a short-lived access token in JSON, held in React memory only. The refresh token is never returned in browser JSON; it is stored in the `HttpOnly` `smsv3_refresh` cookie scoped to `/api/v1/auth`. The readable `smsv3_csrf` cookie is scoped to `/` so protected application routes can supply the matching `X-CSRF-Token` header for refresh, logout, and logout-all requests.
