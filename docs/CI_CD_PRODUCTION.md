...
`DATABASE_URL` may be pooled or direct, depending on the runtime configuration. `DIRECT_URL` is the direct PostgreSQL URL used by Prisma migration commands and must be verified as direct. A session pooler, transaction pooler, PgBouncer endpoint, pooler hostname, pooling query flag, or provider transaction-pool port is rejected. Port 5432 alone is not evidence that a connection is direct.
...
The guard uses provider-aware connection-mode detection. For Supabase, a direct database endpoint and a pooler endpoint are classified separately. The logical pair is normalized to provider identity, project identity, and database identity, so pooled and direct endpoints for the same logical database produce the same fingerprint without including username, password, query secrets, or raw URLs. If provider/project/database identity cannot be proven, the guard fails closed.
...
- `DIRECT_URL` pooled: replace it in the owning database provider with the approved direct endpoint. Do not change a port or hostname by guesswork.
- `DIRECT_URL` mode unknown: use a provider-supported direct endpoint; the production guard does not accept an unverified mode.
